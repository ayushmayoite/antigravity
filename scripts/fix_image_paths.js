  ...
  for (const product of products) {
    ...
    const fixPath = (p) => {
      ...
      if (p.includes('/images/products/imported/') || p.includes('/images/educational/') || p.includes('/images/collaborative/') || p.includes('/products/')) {
        ...
        const newPath = `/images/afc/${newDir}/image-1.webp`;
        
        const localDir = path.join(process.cwd(), 'public', 'images', 'afc', newDir);
        ...
        if (folderExists) {
          ...
          const afcDir = path.join(process.cwd(), 'public', 'images', 'afc');
          const dirs = fs.readdirSync(afcDir);
          const fuzzyMatch = dirs.find(d => d.endsWith(`--${slug}`) || d.includes(`--${slug}-`));
          if (fuzzyMatch) {
            const fuzzyPath = `/images/afc/${fuzzyMatch}/image-1.webp`;
            if (fuzzyPath !== p) {
    ...
    const fixImagesArray = (arr) => {
      ...
      return arr.map((p, idx) => {
        ...
        if (p.includes('/images/products/imported/') || p.includes('/images/educational/') || p.includes('/images/collaborative/') || p.includes('/products/')) {
          ...
          const newPath = `/images/afc/${targetDir}/image-${idx + 1}.webp`;
          const localFile = path.join(process.cwd(), 'public', 'images', 'afc', targetDir, `image-${idx + 1}.webp`);
          ...
          if (fs.existsSync(localFile)) {
            ...
            const finalDir = path.join(process.cwd(), 'public', 'images', 'afc', targetDir);
            if (fs.existsSync(finalDir)) {
               const files = fs.readdirSync(finalDir).filter(f => f.endsWith('.webp'));
               if (files.length > 0) {
                 const fallbackFile = files[idx % files.length];
                 const fallbackPath = `/images/afc/${targetDir}/${fallbackFile}`;
                 if (fallbackPath !== p) {
                   needsUpdate = true;
                 }
                 return fallbackPath;
