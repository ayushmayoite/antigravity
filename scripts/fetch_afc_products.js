...
async function fetchProductUrlsFromCategory(categoryUrl) {
    try {
        ...
        return productUrls.map(url => 
            url.startsWith('http') ? url : `https://www.afcindia.in${url}`
        );
    } catch (error) {
...
async function fetchProductData(productUrl) {
    try {
        ...
        const description = $('meta[name="description"]').attr('content') ||
                           $('p').first().text().trim() ||
                           'Premium furniture solution from AFC India';
        ...
        return {
            ...
            flagshipImage: images.length > 0 ? images[0] : '/images/afc/placeholder.webp',
            ...
            variants: [{
                ...
                galleryImages: images.slice(0, 7).length >= 7 ? 
                    images.slice(0, 7) : 
                    images.concat(Array(7 - images.length).fill('/images/afc/placeholder.webp'))
            }],
            detailedInfo: {
...
async function getAllAfcProducts() {
    console.log('Starting AFC product scraping...');
...
function integrateIntoCatalog(products) {
    ...
    for (const [categoryName, categoryProducts] of Object.entries(productsByCategory)) {
        const category = {
            id: `afc-${categoryName}`,
            name: `AFC ${categoryName.charAt(0).toUpperCase() + categoryName.slice(1)}`,
            description: `Premium ${categoryName} solutions from AFC India`,
            series: [{
                id: `afc-${categoryName}-series`,
                name: `AFC ${categoryName.charAt(0).toUpperCase() + categoryName.slice(1)} Series`,
                description: `High-quality ${categoryName} designed for modern workspaces`,
                products: categoryProducts
            }]
        };
        afcCategories.push(category);
    }
    
    // Add AFC categories to the catalog
    ...
    ${afcCatalogString.substring(1, afcCatalogString.length - 1)},`
        ...
        console.log(`Successfully integrated ${products.length} AFC products into catalog.ts`);
        console.log(`Created ${afcCategories.length} AFC categories`);
    } else {
...
async function main() {
    ...
    try {
        const products = await getAllAfcProducts();
