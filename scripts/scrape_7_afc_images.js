  ...
  for (const selector of selectors) {
    $(selector).each((i, elem) => {
      ...
      if (src && !src.includes('logo') && !src.includes('icon') && !src.includes('avatar') && !src.includes('.svg')) {
        const fullUrl = src.startsWith('http') ? src : `https://www.afcindia.in${src.startsWith('/') ? '' : '/'}${src}`;
        images.add(fullUrl);
      }
    });
  }
...
async function processProduct(product, totalProducts, index) {
  try {
    ...
    // Try multiple possible URLs for AFC India
    const possibleUrls = [
      `https://www.afcindia.in/products/${baseName}`,
      `https://www.afcindia.in/products/${product.name.toLowerCase().replace(/[^a-z0-9]/g, '-')}`,
      `https://www.afcindia.in/products/${baseName.replace(/-/g, '')}`,
      `https://www.afcindia.in/product/${baseName}`
    ];
    ...
    if (!$) {
      console.log(`[${index}/${totalProducts}] ⚠️ Skipped ${product.name} (Not found on AFC)`);
      return false;
    }
    ...
    for (let i = 0; i < imageUrls.length; i++) {
      ...
      const webPath = `/images/afc/${product.slug}/${fileName}`;
...
async function main() {
  console.log('🚀 Starting AFC Image Scraper (min 7 WebP images per product)');
