...
function SeriesLoadingSkeleton() {
  return (
    <div className="container px-6 2xl:px-0 space-y-32 py-32">
      {Array.from({ length: 3 }).map((_, i) => (
        <div
          key={i}
          className="flex flex-col lg:flex-row gap-16 items-center animate-pulse"
        >
          <div className="w-full lg:w-3/5 aspect-4/3 bg-neutral-100 rounded-sm" />
...
async function SeriesContent({
  ...
  const heroImage =
    series.products[0]?.flagshipImage ||
  ...
  return (
    <main className="flex min-h-screen flex-col items-center bg-white selection:bg-neutral-200">
      ...
      <Hero
        ...
        title={
          <>
            <span className="font-serif tracking-tight text-6xl md:text-8xl">
              {series.name}
            </span>
          </>
        }
        subtitle={
          <span className="font-light tracking-wide mt-6 block max-w-2xl mx-auto">
            {series.description ||
              `Discover the architectural elegance of ${series.name}.`}
          </span>
        }
        showButton={true}
      ...
      <section className="container px-6 2xl:px-0 py-32">
        <div className="max-w-4xl mx-auto text-center space-y-12">
          <Reveal>
            <h2 className="typ-h2">
              Designed for{" "}
              <span className="italic text-primary font-serif">Focus.</span>{" "}
              <br />
              Engineered for{" "}
          ...
          <Reveal>
            <p className="text-xl md:text-2xl font-light text-neutral-500 leading-relaxed">
              The {series.name} collection represents the pinnacle of modern
              workspace design. Manufactured with precision by One and Only, and
              curated exclusively for the visionary office environments
              constructed by One and Only.
            </p>
          </Reveal>
        </div>
      </section>
      ...
      <section className="w-full bg-neutral-50 py-32 border-y border-neutral-100">
        <div className="container px-6 2xl:px-0 space-y-32">
          {series.products.map((product: CompatProduct, index: number) => (
            <div
              key={product.id}
              className={clsx(
                ...
                <Reveal delay={0.2}>
                  ...
                  <div className="mt-8 space-y-6 text-neutral-600 font-light text-lg">
                    <p>
                      ...
                      The {product.name} from our {series.name} series is a
                      testament to manufacturing excellence.
                    </p>

                    <ul className="space-y-4 pt-4 border-t border-neutral-200">
                      {(product.detailedInfo?.features?.length > 0
      ...
      <section className="w-full py-32 bg-neutral-900 text-white text-center px-6">
        <div className="max-w-3xl mx-auto space-y-8">
          <h2 className="typ-h2 font-serif">Elevate your workspace.</h2>
          <p className="text-xl text-neutral-400 font-light">
            Our design consultants are ready to assist you in integrating the{" "}
            {series.name} collection into your architectural vision.
          </p>
...
export default async function SeriesPage({
  ...
  return (
    <Suspense fallback={<SeriesLoadingSkeleton />}>
      <SeriesContent categoryId={categoryId} seriesId={seriesId} />
    </Suspense>
  );
}
